...
//        int bufferSize = 128 * 1024;   // 128KB buffer
        int bufferSize = randomMode ? 4 * 1024 : 128 * 1024;    // 隨機模式 4K, 持續模式 128K
...
//        new Random().nextBytes(randomData);
        ...
        // 使用 FileChannel 提高性能
        try (FileOutputStream fileOutputStream = new FileOutputStream(file, false)) {   // false 代表 OverWrite(覆蓋模式)
...
//                long cycleStart = System.nanoTime();
                ...
                } else if (fileChannel.position() + bufferSize > maxFileSize) {
